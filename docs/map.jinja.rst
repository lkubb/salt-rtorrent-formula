...
We will see a quick setup to configure the ``rtorrent`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``rtorrent`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``rtorrent`` formulas under ``/srv/salt/rtorrent/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/rtorrent/parameters/dns:domain/
    ln -s dns:domain /srv/salt/rtorrent/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/rtorrent/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/rtorrent/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``rtorrent`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``rtorrent`` formulas under ``/srv/salt/rtorrent/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/rtorrent/parameters/roles
...
We create a configuration for the role ``rtorrent/server`` in ``/srv/salt/rtorrent/parameters/roles/rtorrent/server.yaml``:
...
We create another configuration for the role ``rtorrent/client`` in ``/srv/salt/rtorrent/parameters/roles/rtorrent/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles rtorrent/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles rtorrent/client
...
.. code-block:: console
    ...
    mv /srv/salt/rtorrent/parameters/dns:domain/  '/srv/salt/rtorrent/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-rtorrent-as-json:
      test.nop:
        - name: {{ rtorrent | json }}
...
Here is an example based on `template-formula/rtorrent/config/file.sls`_:
    ...
    rtorrent-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='rtorrent-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ rtorrent.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``rtorrent`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ rtorrent['winner'] }}
...
.. _template-formula/rtorrent/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/rtorrent/config/file.sls
